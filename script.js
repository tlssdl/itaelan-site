import {STORAGE_KEY,LEGACY_KEY,STATUSES,emptyState,dateKey,weekKey,shiftDate,safeURL,normalizeState,saveTask,updateTask,mergeState,filterTasks,shiftMonth,calendarDays} from './planner-state.js';
...
const views={calendar:'월간 캘린더',week:'주간 계획',all:'전체 계획',unscheduled:'날짜 미정',trash:'휴지통'};
let state=emptyState(),view='calendar',month=today().slice(0,7),calendarDay='',week=weekKey(),day='',editingId=null,pendingImport=null,blocked=false,originalRaw='',toastTimer;
...
function selectedTasks({filtered=true}={}){return filterTasks(state,{view,week,month,day:view==='calendar'?calendarDay:day,query:filtered?$('#search').value:'',status:filtered?$('#status-filter').value:'all'});}
function badge(task){return `<span class="status-badge ${task.status}">${STATUSES[task.status]}</span>`;}
function taskRow(t){
  return `<article class="task-row ${t.status==='done'?'is-done':''}">${t.deleted?'':`<button class="complete-button ${t.status==='done'?'done':''}" data-complete="${escape(t.id)}" aria-pressed="${t.status==='done'}" aria-label="${escape(t.title)} ${t.status==='done'?'완료 취소':'완료 표시'}">${t.status==='done'?'✓':''}</button>`}<div class="task-body"><div class="task-heading"><button class="task-title" data-edit="${escape(t.id)}">${escape(t.title)}</button>${badge(t)}</div>${t.action?`<p class="task-action">${escape(t.action)}</p>`:''}<div class="task-meta">${t.time?`<span class="category-tag">${escape(t.time)}${t.endTime?' – '+escape(t.endTime):''}</span>`:''}${t.location?`<span class="category-tag">⌖ ${escape(t.location)}</span>`:''}${t.category?`<span class="category-tag">${escape(t.category)}</span>`:''}${t.url?`<a class="doc-link" href="${escape(safeURL(t.url))}" target="_blank" rel="noopener noreferrer">↗ ${escape(t.document||'자료 열기')}</a>`:t.document?`<button class="doc-link" data-copy="${escape(t.id)}">▤ ${escape(t.document)} ⧉</button>`:''}${t.deleted?`<button class="restore-button" data-restore="${escape(t.id)}">복원하기</button>`:`<button class="row-edit" data-edit="${escape(t.id)}" aria-label="${escape(t.title)} 수정">수정 ↗</button>`}</div></div></article>`;
}

function renderCalendar(){
  $('#month-title').textContent=new Date(month+'-01T12:00:00').toLocaleDateString('ko-KR',{year:'numeric',month:'long'});
  $('#month-jump').value=month;
  const visible=filterTasks(state,{view:'all',query:$('#search').value,status:$('#status-filter').value});
  const byDate=new Map();for(const t of visible){if(t.date){const list=byDate.get(t.date)||[];list.push(t);byDate.set(t.date,list);}}
  $('#calendar-grid').innerHTML=calendarDays(month).map((key,index)=>{
    const items=byDate.get(key)||[];
    return '<section class="calendar-cell '+(key.slice(0,7)!==month?'outside ':'')+(key===today()?'today-cell ':'')+(key===calendarDay?'selected-cell ':'')+(index%7===6?'sunday':'')+'" aria-label="'+displayDate(key)+'">'+
      '<div class="calendar-cell-heading"><button class="calendar-date" data-calendar-date="'+key+'" aria-label="'+displayDate(key)+' 일정 추가">'+Number(key.slice(-2))+'</button>'+(key===today()?'<span class="today-label">오늘</span>':'')+'</div>'+
      '<div class="calendar-items">'+items.slice(0,3).map(t=>'<button class="calendar-event '+(t.status==='done'?'completed':'')+'" data-edit="'+escape(t.id)+'" title="'+escape((t.time||'종일')+' '+t.title)+'"><span class="event-time">'+escape(t.time||'종일')+'</span><span class="event-title">'+escape(t.title)+'</span></button>').join('')+'</div>'+
      (items.length>3?'<button class="calendar-more" data-calendar-list="'+key+'">+'+(items.length-3)+'개 더 보기</button>':'')+
      '<button class="calendar-add" data-calendar-date="'+key+'" aria-label="'+displayDate(key)+' 일정 추가">＋</button></section>';
  }).join('');
}

function render(){
  const focused=document.activeElement,attr=['data-complete','data-edit','data-restore','data-day','data-calendar-date','data-calendar-list'].find(a=>focused?.hasAttribute(a)),value=attr?focused.getAttribute(attr):null;
  ...
  $('#page-title').textContent=view==='trash'?'휴지통':view==='unscheduled'?'날짜는 나중에.':view==='calendar'?'나의 캘린더':'나의 공부 계획';
  $('#page-subtitle').textContent=view==='trash'?'지운 계획을 다시 꺼낼 수 있어요.':view==='unscheduled'?'언젠가 공부하고 싶은 것들을 적어두세요.':view==='calendar'?'공부할 일도, 약속도 한곳에.':'오늘 할 공부부터, 다음 주 목표까지.';
  $('#quick-panel').hidden=view==='trash'||view==='calendar';$('#calendar-panel').hidden=view!=='calendar';$('#new-btn').textContent=view==='calendar'?'＋ 일정 추가':'＋ 계획 추가';$('#new-btn').hidden=view==='trash';$('#week-controls').hidden=view!=='week';$('.overview').hidden=view==='trash'||view==='calendar';$('.plan-layout').classList.toggle('single-column',view==='trash'||view==='calendar');
  ...
  const title=view==='calendar'?(calendarDay?displayDate(calendarDay)+' 일정':'이 달의 일정'):view==='week'?(day?displayDate(day)+' 계획':week===weekKey()?'이번 주 계획':'선택한 주 계획'):views[view];
  ...
  const overview=view==='week'?weekTasks:filterTasks(state,{view,month}),done=overview.filter(t=>t.status==='done').length,percent=overview.length?Math.round(done/overview.length*100):0;
...
function defaultDate(){return view==='calendar'?(calendarDay||(month===today().slice(0,7)?today():month+'-01')):view==='unscheduled'?'':view==='week'?(day||(week===weekKey()?today():week)):today();}
function resetQuickDate(){$('#quick-date').value=defaultDate();}
function changeView(next){view=Object.hasOwn(views,next)?next:'calendar';day='';calendarDay='';$('#search').value='';$('#status-filter').value='all';resetQuickDate();render();}
function reveal(task){$('#search').value='';$('#status-filter').value='all';if(view==='calendar'&&task.date){month=task.date.slice(0,7);calendarDay=task.date;}else if(view!=='all'){view=task.date?'week':'unscheduled';if(task.date){week=weekKey(new Date(task.date+'T12:00:00'));day=task.date;}history.replaceState(null,'','#'+view);}resetQuickDate();render();}
function openEditor(id=null,date=null){
  const t=id?state.tasks.find(t=>t.id===id):{title:'',date:date||defaultDate(),time:'',endTime:'',location:'',status:'todo',category:'',action:'',document:'',url:'',priorRecords:[]};if(!t)return;
  editingId=id;$('#editor-heading').textContent=t.deleted?'휴지통의 계획':id?'내용 수정':view==='calendar'?'일정 추가':'계획 추가';
  if(t.deleted)$('#editor-heading').textContent='휴지통의 계획';
  $('#edit-title').value=t.title;$('#edit-title').setCustomValidity('');$('#edit-date').value=t.date;$('#edit-time').value=t.time||'';$('#edit-end-time').value=t.endTime||'';$('#edit-location').value=t.location||'';$('#edit-title-label').textContent='제목';$('#edit-status').value=t.status;$('#edit-category').value=t.category;$('#edit-action').value=t.action;$('#edit-document').value=t.document;$('#edit-url').value=t.url;$('#document-fields').open=!!(t.document||t.url);$('#edit-error').textContent='';$('#trash-btn').hidden=!id||t.deleted;
...
$('#editor-form').addEventListener('submit',e=>{
  ...
  const time=$('#edit-time').value,endTime=$('#edit-end-time').value;if(endTime&&(!time||endTime<=time)){$('#edit-error').textContent='종료 시간은 시작 시간보다 늦게 입력해주세요. 날짜를 넘기는 일정은 날짜별로 나눠 적어주세요.';return;}
  const old=state.tasks.find(t=>t.id===editingId),task={...old,id:editingId||'task-'+crypto.randomUUID(),title,date:$('#edit-date').value,time,endTime,location:$('#edit-location').value.trim(),category:$('#edit-category').value.trim(),status:$('#edit-status').value,action:$('#edit-action').value.trim(),document:$('#edit-document').value.trim(),url};
...
$('#prev-month').addEventListener('click',()=>{month=shiftMonth(month,-1);calendarDay='';render();});
$('#next-month').addEventListener('click',()=>{month=shiftMonth(month,1);calendarDay='';render();});
$('#current-month').addEventListener('click',()=>{month=today().slice(0,7);calendarDay=today();render();});
$('#month-jump').addEventListener('change',e=>{if(/^\d{4}-\d{2}$/.test(e.target.value)){month=e.target.value;calendarDay='';render();}});
$('#month-overview').addEventListener('click',()=>{calendarDay='';render();});
...
document.addEventListener('click',async e=>{
  ...
  if(b.dataset.calendarDate){calendarDay=b.dataset.calendarDate;render();openEditor(null,calendarDay);return;}
  if(b.dataset.calendarList){calendarDay=b.dataset.calendarList;render();$('#task-list').scrollIntoView?.({behavior:'smooth',block:'nearest'});return;}
  ...
  if(b.dataset.complete){const t=state.tasks.find(t=>t.id===b.dataset.complete);if(t&&commit(updateTask(state,t.id,{status:t.status==='done'?'studying':'done'})))toast(t.status==='done'?'진행 중으로 변경했어요.':'완료했어요. 수고했어요!');return;}
